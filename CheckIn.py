...
def putrequests(api, headers, data):
    return requests.put(url=api, headers=headers, data=data).json()
        
def postrequests(api):
    return requests.post(url=api, headers=headers).json()
    
def getrequests(api, headers):
...
def task():
    ...
    result = requests.put(
        "https://api.sfacg.com/user/signInfo", headers=headers).json()
    responed = requests.put(
        'https://api.sfacg.com/user/readingtime', headers=TimerHeaders, data=ListenData)
    # print(result.json())
    # print(headers)
    # print(responed)
    ...
    if result['status']['msg'] == '您今天已经签过到了,请明天再来':
        ...
        print("开始签到和完成任务")
        # 领取任务
        postrequests('https://api.sfacg.com/user/tasks/4')
        postrequests('https://api.sfacg.com/user/tasks/5')
        postrequests('https://api.sfacg.com/user/tasks/17')
        for i in range(3):
            r = putrequests('https://api.sfacg.com/user/readingtime', TimerHeaders, ReadData)
            print(r)
            time.sleep(0.5)
            putrequests('https://api.sfacg.com/user/tasks/5', headers, ReadData)
            putrequests('https://api.sfacg.com/user/tasks/4', headers, ReadData)
            putrequests('https://api.sfacg.com/user/tasks/17', headers, ReadData)
